...
- **A colour, font or radius outside `LoupeTheme.swift` is a build failure.**
  ...
  the overlay is a visitor again in exactly one control - which reads as a bug rather
  than as a theme, and only shows up inside the host nobody testing Loupe is running.
  A value with no token yet gets one, in `LoupeTheme.swift` and `DESIGN.md`, in the
  same change.
...
- Snapshots only cover macOS. For iPad and iPhone, build the app and drive it with the
  ...
  `occlusion`, `lowpick`, `settings`, `zero`, `emptytray`). Flags rather than scenes:
  `--fresh-session` (throw away the saved tray first), `--offer-teardown` (a button
  that calls `Loupe.stop()`), `--host-theme` (the demo hands Loupe its own tokens, so
  the overlay wears an indigo accent and rounder panels instead of Loupe's own).
